...
from . import log
from .function import download_ham_sat, data2Tle, calculate
from .path import *
from .utils import *
...
async def refer_sat_(bot: Bot, event: GroupMessageEvent, state: T_State, args: Message = CommandArg()):
    ...
    if group in data:
        if qq in data[group]:
            await refer_sat_subs.send(f"{qq}在本群订阅了以下卫星：\n")
            reply = ""
...
async def _(bot: Bot, event: MessageEvent, state: T_State, args: Message = CommandArg()):
    ...
    try:
        ...
        add = 0
        await specified.finish("参数不正确，请重新输入\n  卫星名称 分钟\n例：【SO-50 60】 \n(SO-50 60分钟后的仰角情况)")
